...
def InfoAdicional():
    ...
    messagebox.showinfo("Venatana de prueba de ventanas emergentes",
                        "Esta es una app de prueba para probar todo lo aprendido")


# Ventana Emergente de Error
def ErrorDesconocido():
    messagebox.showerror(
        "Fatal Error", "Lo sentimos algo salio mal intenta mas tarde")
...
def Reintentar():
    ...
    valor = messagebox.askretrycancel(
        "Reintentar", "¿ No se pudo abrir el archivo, Desea reintentarlo ?")
...
Cuadrotexto = Text(raiz, width=35, height=8)
...
def opcionesDeportes():
    ...
    Message.config(text=opcionescogida)
